var FormulaValues = (function (exports) {
	'use strict';
	...
	const evalWithSafeEnvironment = (function () {
	    ...
	    const __createObjectDate = (date, dateFormat) => {
	        if (dateFormat === 'HH:mm:ss') {
	            ...
	            const parsedDateFormat = dayjs_min()
	            ...
	            return dayjs_min(date, dateFormat);
	        }
	    };
	    // eslint-disable-next-line @typescript-eslint/no-unused-vars
	    ...
	    const formatDate = (date, format, isUTC) => {
	        ...
	        if (isUTC) {
	            return dayjs_min.utc(date).local().format(format);
	        }
	        else {
	...
	const INVALID_VARIABLE_REGEX = new RegExp(patterns.invalidVariable);
	class Variable {
	    constructor(text) {
	...
	const VARIABLE_REGEX$1 = new RegExp(patterns.variable, 'g');
	class CompiledExpression {
	    constructor(rules, expression) {
	        ...
	        this._parsedExpression = expression.replace(VARIABLE_REGEX$1, (_match, variableText) => {
	...
	const CLEANING_RULES$1 = [
	    {
	        pattern: '\'',
	        replacement: '\\\''
	    }
	];
	const VARIABLE_REGEX = new RegExp(patterns.variable);
	const PARSED_EXPRESSION_REGEX = new RegExp(patterns.parsedExpression, 'g');
	class ConcatenatedText extends CompiledExpression {
	    constructor(expression = '') {
	        super(CLEANING_RULES$1, expression);
	    }
	...
	class DefaultValue extends CompiledExpression {
	    constructor(expression) {
	        super([], '');
	...
	class Formula extends CompiledExpression {
	    constructor(expression) {
